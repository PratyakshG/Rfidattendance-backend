...
export const getAdminDashboard = async (req, res) => {
  try {
    ...
    const totalEmployees =
      (await User.countDocuments({ role: "Employee" })) || 0;

    // Get today's attendance
    const todayAttendance =
      (await Attendance.find({ date: today }).populate(
        "user",
        "name uid email",
      )) || [];
    ...
    // Calculate present employees on if not marked as ABSENT
    const presentEmployees = todayAttendance.filter(
      (a) => a.status !== "ABSENT" && a.user.role !== "ADMIN",
    );

    // Calculate absent employees on if marked as ABSENT
    ...
    try {
      complaintStats = {
        ...
        inProcess:
          (await Complaint.countDocuments({ status: "IN_PROGRESS" })) || 0,
